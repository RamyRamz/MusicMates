...
/**
 * Class to handle Participants/ Program registration
 */
...
class PP
{
  ...
  */
  ...
  public function __construct( $data=array() ) {
    ...
    if ( isset( $data['PP_ID'] ) ) $this->PP_ID = (int) $data['PP_ID'];
  ...
  */
  ...
  public static function getById( $id ) {
    ...
    $sql = "SELECT * FROM PartProg WHERE PP_ID = :id";
    $st = $conn->prepare( $sql );
    $st->bindValue( ":id", $id, PDO::PARAM_INT );
    $st->execute();
    $row = $st->fetch();
    $conn = null;
    if ( $row ) return new PP( $row );
  }
  ...
  */
  ...
  public static function getList( $numRows=1000000, $order="PP_ID ASC" ) {
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "SELECT SQL_CALC_FOUND_ROWS * FROM PartProg
  ...
  */
  ...
  public function insert() {
    ...
    if ( !is_null( $this->id ) ) trigger_error ( "PP::insert(): Attempt to insert an PP object that already has its ID property set (to $this->id).", E_USER_ERROR );

    // Insert the Participant
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "INSERT INTO PartProg ( participantID, programID ) VALUES ( :participantID, :programID )";
    $st = $conn->prepare ( $sql );
    $st->bindValue( ":participantID", $this->participantID, PDO::PARAM_INT );
    $st->bindValue( ":programID", $this->programID, PDO::PARAM_INT );

    $st->execute();
    $this->PP_ID = $conn->lastInsertId();
    $conn = null;
  }
  ...
  */
  ...
  public function update() {
    ...
    if ( is_null( $this->PP_ID ) ) trigger_error ( "PP::update(): Attempt to update a PP object that does not have its ID property set.", E_USER_ERROR );
   
    // Update the Participant
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $sql = "UPDATE PartProg SET participantID=:participantID, programID = :programID WHERE PP_ID = :PP_ID";
    $st = $conn->prepare ( $sql );
    $st->bindValue( ":participantID", $this->participantID, PDO::PARAM_INT );
    $st->bindValue( ":programID", $this->programID, PDO::PARAM_INT );
    $st->execute();
  ...
  */
  ...
  public function delete() {
    ...
    // Does the Participant object have an ID?
    if ( is_null( $this->PP_ID ) ) trigger_error ( "PP::delete(): Attempt to delete a PP object that does not have its ID property set.", E_USER_ERROR );

    // Delete the Participant
    $conn = new PDO( DB_DSN, DB_USERNAME, DB_PASSWORD );
    $st = $conn->prepare ( "DELETE FROM PartProg WHERE PP_ID = :id LIMIT 1" );
    $st->bindValue( ":id", $this->PP_ID, PDO::PARAM_INT );
    $st->execute();
    $conn = null;
  }
